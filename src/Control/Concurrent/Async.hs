...
-- | A forked task resulting from a call to 'forkTask'. Can be synchronised
-- with the current thread with 'joinTask'.
...
forkTask action = do mvar <- newEmptyMVar
...
-- cannot be restarted.
--
-- If the task fails, then this function will fail as well (as opposed to
-- blocking forever).
...
-- completed. Use 'forkTask' to prevent this from happening.
--
...
asyncAny :: [IO a] -> IO a
asyncAny tasks = do mvar <- newEmptyMVar
...
asyncBoth a b = do bT <- forkTask b
                   a' <- a
                   b' <- joinTask bT
                   return (a', b')
